...
import { PublishService, PublishError } from '@/app/foundation/core/services/publish.service';
...
@Component({
  ...
  standalone: true,
  imports: [ToastModule, ButtonModule, TooltipModule, Domains, Branding, UIStyleDesignerComponent],
...
export class AppConfigurationComponent {
  ...
  readonly navSections: NavSection[] = [
    { key: 'branding',  label: 'Branding',      icon: 'pi pi-image',           description: 'Logo, name, metadata & social' },
    { key: 'setup',     label: 'Project Setup',  icon: 'pi pi-box',             description: 'Domain & configuration' },
    { key: 'ui-style',  label: 'UI Style',       icon: 'pi pi-objects-column',  description: 'Components style designer' },
  ];
  ...
  setSection(key: SectionKey): void { this.activeSection.set(key); }
  isSectionDone(key: SectionKey): boolean { return this.doneSections().has(key); }
  ...
  readonly validateState = signal<'idle' | 'validating' | 'valid' | 'invalid'>('idle');
  ...
  readonly publishState = signal<'idle' | 'validating' | 'publishing' | 'done' | 'error'>('idle');
  ...
  readonly isPublishing = computed(() =>
    this.publishState() === 'validating' || this.publishState() === 'publishing'
  );
