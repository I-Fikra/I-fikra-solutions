...
const SHAPE_RADIUS:  Record<ComponentShape, string> = { sharp: '2px', rounded: '6px', soft: '14px', pill: '999px' };
...
export function tokensFromCustom(c: CustomPersonality): Partial<PersonalityTokens> {
  ...
  return {
    ...
    fontSizeBase: typ.fontSizeBase,
    scaleRatio: typ.scaleRatio,
    responsiveMinWidth: typ.responsiveMinWidth,
    responsiveFontSize: typ.responsiveFontSize,
    responsiveScale: typ.responsiveScale,
    bodyWeight: typ.bodyWeight,
    bodyLineHeight: typ.bodyLineHeight,
    letterSpacing: typ.bodyLetterSpacing,
    bodyColor: typ.bodyColor,
    bodyBackground: typ.bodyBackground,
...
export class ThemePersonalityService {
  ...
  applyPersonality(tokens: Partial<PersonalityTokens>): void {
    ...
    if (tokens.scaleRatio)         root.style.setProperty('--app-type-scale-ratio',      tokens.scaleRatio);
    if (tokens.responsiveMinWidth) root.style.setProperty('--app-responsive-min-width',  tokens.responsiveMinWidth + 'px');
    if (tokens.responsiveFontSize) root.style.setProperty('--app-responsive-font-size',  tokens.responsiveFontSize + 'px');
    if (tokens.responsiveScale)    root.style.setProperty('--app-responsive-scale',      tokens.responsiveScale);
    ...
    if (tokens.bodyBackground)     root.style.setProperty('--app-body-background',       tokens.bodyBackground);
