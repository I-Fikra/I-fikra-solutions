...
@Component({
  ...
  template: `
    <div class="app-popup-shell-inner py-6 px-5 w-80">
  ...
  `,
  host: {
    class: 'app-popup-shell hidden absolute top-14 end-0 origin-top z-50'
  }
})
export class AppConfigurator {
  ...
  updateColors(event: any, type: string, color: any) {
    if (type === 'primary') {
      ...
      this.settingsService.updatePreference('surface', color.name);
    }
  ...
  onPresetChange(event: any) {
    this.settingsService.updatePreference('theme', event);
  ...
  onMenuModeChange(event: string) {
    this.settingsService.updatePreference('menuMode', event as any);
  }
}
